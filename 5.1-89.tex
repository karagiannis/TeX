...
\begin{flalign*}
\frac{dp}{p}+n\cdot\frac{dV}{V}&=\frac{dT}{T} & \\
V\cdot dp + n\cdot p\cdot dV &= p\cdot V \frac{dT}{T}
\end{flalign*}
...
\begin{flalign*}
(n-1)\cdot p\cdot dV &= -m\cdot R\cdot dT & \text{vilket ger}\\
p\cdot dV &=-\frac{m\cdot R\cdot dT}{n-1} 
\end{flalign*}
Vi kan nu beräkna värmemängdsändringen per massenhet $q$ för en polytrop process
\begin{flalign*}
dq &= du + p\cdot dv &\\
dq &= \bar{c}_v\cdot dT -\frac{R\cdot dT}{n-1} &\\
q_{34} &=\int_3^4  \bar{c}_v\cdot dT -\frac{R\cdot dT}{n-1} &\\
       &= \bar{c}_v\cdot (T_4-T_3) -\frac{R\cdot (T_4-T_3)}{n-1} &\\
       &=\bar{c}_v\cdot (T_4-T_3) -\frac{\bar{c}_v\cdot (\kappa -1) \cdot (T_4-T_3)}{n-1} &\\
       &=\bar{c}_v\cdot (T_4-T_3)\cdot\bigg[\frac{(n-1)-(\kappa -1)}{n-1}\bigg]&\\
       &=\bar{c}_v\cdot (T_4-T_3)\cdot\frac{(n-\kappa )}{n-1}&\\
\end{flalign*}
...
s_4 - s_1 &=  \bar{c}_v\cdot ln\bigg(\frac{T_4}{T_1}\bigg) &\\
\end{flalign*}
Likaså har vi därför för isokoren $(2)$ till $(3)$
\begin{flalign*}
s_3 - s_2 &=  \bar{c}_v\cdot ln\bigg(\frac{T_3}{T_2}\bigg) &\\
\end{flalign*}
För den polytropa processen $(3)$ till $(4)$ så har vi
\begin{flalign*}
ds &= \bigg( \frac{dq}{T}\bigg)_{rev} &\\ 
   &=\frac{du}{T} + \frac{p\cdot dv}{T} &\\
   &= \bar{c}_v\cdot \frac{dT}{T} -\frac{R}{n-1}\cdot \frac{dT}{T} &\\
\int_3^4 ds &= \int_3^4 \bar{c}_v\cdot \frac{dT}{T} -\frac{R}{n-1}\cdot \frac{dT}{T} &\\
s_4 - s_3 &=  \bar{c}_v\cdot ln\bigg(\frac{T_4}{T_3}\bigg)-\frac{R}{n-1}\cdot ln\bigg(\frac{T_4}{T_3}\bigg) &\\
          &=\bar{c}_v\frac{n-\kappa}{n-1}\cdot ln\bigg(\frac{T_4}{T_3}\bigg) &\\
\end{flalign*}
...
Nu tar vi e-upphöjt.
\begin{flalign*}
\frac{T_4}{T_1} &= \bigg(\frac{T_4}{T_3}\bigg)^{\frac{n-\kappa}{n-1}}\cdot \frac{T_3}{T_2} &\\
\end{flalign*}
